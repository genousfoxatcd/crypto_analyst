...
更新历史：
- 2026-04-09: 新增墨黑色主题、A股电力新能源ETF模块、19:00定时推送

使用方法：
1. 手动运行：python daily_briefing_generator.py
2. 定时任务：
   - Linux/Mac: crontab -e → 0 19 * * 1-5 /usr/bin/python3 /workspace/docs/daily_briefing_generator.py
   - Windows: 使用任务计划程序

作者：MiniMax Agent
日期：2026-04-07 (更新: 2026-04-09)
"""
...
<head>
    ...
    <title>每日财经简报 {date} | Aliya财经追踪系统</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .a-share-power .section-title {{ border-color: #00ff88; color: #00ff88; }}
        .analysis .section-title {{ border-color: #fd79a8; color: #fd79a8; }}
        ...
        .tag-gold {{ background: rgba(255,215,0,0.15); color: #ffd700; border: 1px solid rgba(255,215,0,0.3); }}
...
<body>
    <div class="container">
        <header class="header">
            <h1>每日财经简报</h1>
            <p class="subtitle">Aliya 财经追踪系统 | 全球宏观 · 市场动态 · 投资策略</p>
        ...
        <footer class="footer">
            <p style="font-size: 1.1em; color: #555;">
                <strong style="color: #666;">Aliya</strong> 财经追踪系统 | 全球宏观 · 市场动态 · 投资策略
            </p>
            <p class="disclaimer">
            ...
            <div class="email-notice">
                <strong>推送说明：</strong>本简报将于每日19:00自动生成并推送至您的邮箱
            </div>
        </footer>
    </div>
</body>
...
def collect_data():
    """收集财经数据"""
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 开始收集财经数据...")
    data = FinanceData()
    ...
    data.asia_market = {
        ...
        "hang_seng": {"value": "待更新", "change": "-", "status": "关注今日"}
    }

    # A股电力和新能源ETF
    data.power_etf = {
        "green_power_index": {"change": "-0.91%", "name": "国证绿色电力指数"},
        ...
        "green_energy_etf": {"code": "562010", "name": "绿色能源ETF", "trend": "覆盖电池/光伏/电力"}
    }
...
def generate_html(data):
    ...
    content = f"""
        ...
        <!-- A股电力和新能源ETF -->
        <section class="section-card a-share-power">
            ...
            <div class="highlight-box" style="border-color: rgba(0,255,136,0.3);">
                ...
                <table class="data-table" style="margin-top: 15px;">
                    ...
                    <tbody>
                        <tr><td>绿色电力ETF嘉实</td><td>159625</td><td class="negative">{data.power_etf['green_power_etf']['trend'].split('，')[0]}</td><td>{data.power_etf['green_power_etf']['trend']}</td></tr>
                        <tr><td>绿电ETF华夏</td><td>562550</td><td>-</td><td>{data.power_etf['green_power_huaxia']['trend']}</td></tr>
                        <tr><td>电力ETF汇添富</td><td>516370</td><td class="negative">{data.power_etf['power_etf']['change']}</td><td>股息率{data.power_etf['power_etf']['dividend']}，高股息代表</td></tr>
                        <tr><td>储能电池ETF易方达</td><td>159566</td><td class="neutral">{data.power_etf['storage_etf']['trend']}</td><td>储能需求旺盛</td></tr>
                        <tr><td>创业板新能源ETF国泰</td><td>159387</td><td class="positive">{data.power_etf['new_energy_etf']['change']}</td><td>储能含量{data.power_etf['new_energy_etf']['storage']}</td></tr>
                    </tbody>
                </table>
            </div>
