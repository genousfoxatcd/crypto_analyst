...
数据来源层：
  ...
  合约    : Binance Futures API（OI / 资金费率 / 多空比 / 吃单比 / 4h K线 → BB）
  清算预估: OI + 多空比 + ATR → 插针危险区间
  ETF流量 : WebSearch 实时摘要
...
OUT_DIR = Path("/Users/alex/hermes_claud/super_mario/crypto-signal")
...
def estimate_liquidation_zones(price: float, contract: dict, tech: dict) -> dict:
    ...
    # 估算多头集中清算区（向下插针目标）
    liq_long = {
        "5x":  round(price * (1 - 1/5  + 0.005), 2),
        "10x": round(price * (1 - 1/10 + 0.005), 2),
        "20x": round(price * (1 - 1/20 + 0.005), 2),
    }
    # 估算空头集中清算区（向上插针目标）
    liq_short = {
        "5x":  round(price * (1 + 1/5  - 0.005), 2),
...
def run(output_json: str | None = None, output_md: str | None = None) -> dict:
    ...
    for coin, d in prices.items():
        print(f"  {coin:5} ${d['price']:>12,.4f}  {d['change_24h']:+.2f}%  "
              f"  源:{d['source_count']}  偏差:{d['max_dev_pct']}%  {d['quality']}")
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        for f in as_completed(futs):
            ...
            bsr = d.get("taker_buy_sell_ratio")
            oi_m = d.get("open_interest_usdt", 0) / 1e6
            print(f"  {coin:5}  FR:{fr*100:+.4f}%  L/S:{ls:.3f}  TakerBSR:{bsr:.3f}  OI:${oi_m:.1f}M")

    # ③ K线 + 布林带（并发）
    print("\n[3/5] 4h K线 + 布林带 + ATR（24h资金流）...")
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        for f in as_completed(futs):
            ...
            t = tech_data[coin]
            print(f"  {coin:5}  BB:[{t.get('bb_lower'):.2f}|{t.get('bb_mid'):.2f}|{t.get('bb_upper'):.2f}]"
                  f"  pos:{t.get('price_vs_bb_pct'):.0f}%  ATR:{t.get('atr_pct'):.2f}%  "
                  f"  买流:{t.get('flow_24h_buy_pct'):.1f}%")
...
def _write_md_report(data: dict, path: str):
    ...
    for coin, d in data["prices"].items():
        ...
        lines.append(f"| {coin} | ${d['price']:,.4f} | {sign}{abs(d['change_24h']):.2f}% | {d['source_count']} |")
...
if __name__ == "__main__":
    ...
    run(output_json=json_out, output_md=md_out)
